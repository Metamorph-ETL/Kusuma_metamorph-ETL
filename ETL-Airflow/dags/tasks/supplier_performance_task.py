...
def m_load_suppliers_perfomance():
    try:
        ...
        # SQ_Shortcut_To_Sales - Reads data from 'raw.sales' table
        ...
        # SQ_Shortcut_To_Products - Reads data from 'raw.products' table
        SQ_Shortcut_To_Products = read_from_postgres(spark, "raw.products")\
            .select("PRODUCT_ID", "SUPPLIER_ID", "PRODUCT_NAME", "SELLING_PRICE")
        log.info("Data Frame : 'SQ_Shortcut_To_Products' is built")

        # SQ_Shortcut_To_Suppliers - Reads data from 'raw.suppliers' table
        ...
        # FIL_Sales_Cancelled - Filter out cancelled sales
        ...
        # JNR_Sales_Products - Join sales with products
        JNR_Sales_Products = FIL_Sales_Cancelled\
            .join(SQ_Shortcut_To_Products, on="PRODUCT_ID", how="left")\
            .select(
        ...
        # JNR_Products_Suppliers - Join products with suppliers and compute revenue
        JNR_Products_Suppliers = JNR_Sales_Products\
            .join(SQ_Shortcut_To_Suppliers, on="SUPPLIER_ID", how="right")\
        ...
        # AGG_TRANS_Product - Aggregate revenue, quantity, and product count
        ...
        # RNK_Suppliers - Rank to find top-selling product
        RNK_Suppliers = Window.partitionBy("SUPPLIER_ID").orderBy(col("REVENUE").desc(), col("PRODUCT_NAME"))

        # Top_Product_df - Get top-selling product per supplier
        ...
        # Shortcut_To_Supplier_Performance_Tgt - Prepare final target dataset
        Shortcut_To_Supplier_Performance_Tgt = SQ_Shortcut_To_Suppliers\
            .join(AGG_TRANS_Product, on="SUPPLIER_ID", how="left")\
            .join(Top_Product_df, on="SUPPLIER_ID", how="left")\
            ...
            .select(
                ...
                .otherwise(col("TOP_SELLING_PRODUCT")).alias("TOP_SELLING_PRODUCT")
            )\
            .fillna({
                "TOTAL_REVENUE": 0,
        ...
        # Load final DataFrame to PostgreSQL
